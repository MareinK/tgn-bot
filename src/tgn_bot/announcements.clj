...
(defn location-string
  [event]
  (when (:location event)
    (let [locations-string (->> (:location-regexes config)
                                (filter
                                  (fn [{:keys [regex]}]
      ...
      (when (not (str/blank? locations-string))
        (format (get-in config [:messages :daily-location])
                locations-string)))))
  ...
  (doseq [event (calendar-api/get-events-days 0 1)]
    (when-let [message (daily-message event)]
      (messaging/create-message! (:rest @state)
                                 (get-in config
                                         [:channel-ids :daily-announcements])
                                 :content
                                 message))))
  ...
  (str/join "\n"
            (for [event events]
              (let [name (util/remove-prefix (:summary event) "TGN ")
                    start (:start event)]
                (if (:dateTime start)
                  (let [date (-> (java-time/instant (:dateTime start))
                                 (java-time/local-date "Europe/Amsterdam"))
                        date-str (java-time/format "d/M" date)
                        time (-> (java-time/instant (:dateTime start))
                                 (java-time/local-time "Europe/Amsterdam"))
                        time-str (java-time/format "H:mm" time)]
                    (format "- %s: %s vanaf %s"
                            date-str
...
(defn bi-weekly-message
  [events-one events-two]
  (format (get-in config [:messages :bi-weekly])
