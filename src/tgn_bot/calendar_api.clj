...
(defn get-events-days
  [skips days]
  (let [today (-> (java-time/local-time 0 0)
                  (java-time/zoned-date-time (java-time/zone-id
                                               "Europe/Amsterdam"))
                  java-time/instant)
        time-min (java-time/plus today (java-time/period skips :days))
        time-max (java-time/plus time-min (java-time/period days :days))]
    (-> (api-req {:url (str "https://www.googleapis.com/calendar/v3/calendars/"
                            (:calendar-id config)
                            "/events"
...
(comment (get-events-days 0 1) (get-events-days 0 14) (get-events-days 14 14))
