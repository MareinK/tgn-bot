...
(defn channel-history
  ([channel-id] (channel-history channel-id nil))
  ([channel-id before-message-id]
   (let [messages @(apply messaging/get-channel-messages!
     ...
     (when (seq messages)
       (lazy-cat messages
                 (channel-history channel-id (:id (last messages))))))))

(defn active-user-ids-since
  [cutoff-time]
...
(defn inactive-users-since
  ...
  (let [active-user-ids (active-user-ids-since cutoff-time)]
    (->> @(messaging/list-guild-members! (:rest @state)
                                         ...
                                         :limit
                                         1000)
         (remove #(or (contains? active-user-ids (get-in % [:user :id]))
                      (some-> (:joined-at %)
                              (java-time/instant)
                              (java-time/after? cutoff-time)))))))

(comment (for [days [(* 30 36) (* 30 30) (* 30 24) (* 30 18) (* 30 12) (* 30 6)
                     (* 30 3) (* 30 2) 30 15 2 1 0]]
           (let [cutoff-time (java-time/instant (- (System/currentTimeMillis)
                                                   (* 1000 60 60 24 days)))]
