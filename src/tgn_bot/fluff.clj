...
(defn random-greeting
  [user]
  (str (rand-nth (:fluff-greetings config))
       ", "
...
(defn random-answer
  [user]
  (str (formatting/mention-user user)
       " "
       (util/weighted-random-nth (:fluff-answers config))))

(defn respond-to-bot-mention
  [{:keys [channel-id author content]}]
